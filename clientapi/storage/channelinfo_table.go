...
func (s *channelInfoStatements) selectChannelCountByChannelID(ctx context.Context, channeID, participant, partner string) (
	channelcount int64,
	err error) {
	stmt := s.selectChannelCountByChannelIDStmt
	err = stmt.QueryRow(channeID, participant, partner).Scan(&channelcount)
	if err != nil {
...
func (s *channelInfoStatements) initChannelInfo(ctx context.Context,
	...
	count, err := s.selectChannelCountByChannelID(nil, channeID, participant, partner)
	if err != nil {
		return
	}
	if count == 0 {
